...
class Actor:
    ...
    def move(self, game_: 'Game', dx: int, dy: int) -> bool:
        ...
        if not(0 <= new_x < game_.x_tiles and 0 <= new_y < game_.y_tiles):
            return False

        for actor in game_.get_actors():
            if (actor.x == new_x) and (actor.y == new_y):
                if actor.is_push():
                    if not actor.move(game_, dx, dy):
...
class Flag(Character):
    """
    Class representing the flag in the game.
    """
    def __init__(self, x: int, y: int) -> None:
...
class Is(Block):
    ...
    def update(self, up: Optional[Actor],
        ...
        if isinstance(up, Subject) and isinstance(down, Attribute):
            x = "{} is{}".format(up.word.capitalize(), down.word.capitalize())
            rule[0] = x

        if isinstance(left, Subject) and isinstance(right, Attribute):
            y = "{} is{}".format(left.word.capitalize(),
                                 right.word.capitalize())
            rule[1] = y
